...
const handleSaveMatrix = async () => {
  ...
  // 1. GOM DỮ LIỆU TỪ selectedTopics THÀNH CHUỖI ĐỂ KHỚP ĐỊNH DẠNG CŨ
  // Lọc bỏ những dòng chưa chọn chuyên đề
 const validRows = selectedTopics.filter(t => t.idcd !== "" && t.idcd !== undefined);
  ...
  try {
    ...
    if (result.status === "success") {
      ...
      setIsMatrixOpen(false); // Đóng modal khi thành công
    } else {
      alert("⚠️ Lỗi Script: " + result.message);
    }
  } catch (e) {
    console.error(e);
    alert("❌ Lỗi kết nối! Kiểm tra lại mạng hoặc URL Script.");
  } finally {
...
{/* X.Quiz */}
    ...
    <div className="bg-white w-full max-w-6xl max-h-[95vh] overflow-hidden rounded-2xl shadow-2xl flex flex-col animate-in fade-in zoom-in duration-200">
      ...
      <div className="bg-gradient-to-r from-blue-800 to-indigo-900 p-4 text-white">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="space-y-1">
            ...
            <input value={idgv} onChange={e => setIdgv(e.target.value)} className="w-full bg-yellow-400/10 border border-yellow-400/40 rounded-lg px-3 py-2 text-sm outline-none font-bold text-yellow-300" placeholder="GV-XXX"/>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto p-4 bg-gray-50 custom-scrollbar">
        <div className="min-w-[950px]">
          {/* Header Bảng */}
          <div className="grid grid-cols-[2.5fr_repeat(9,1fr)] gap-2 mb-3 text-center">
            <div className="text-left font-bold text-gray-400 text-[10px] uppercase">Chuyên đề</div>
            <div className="col-span-3 py-1 bg-blue-600 text-white rounded-t-lg text-[10px] font-bold uppercase tracking-widest">Phần I (Trắc nghiệm)</div>
            <div className="col-span-3 py-1 bg-emerald-600 text-white rounded-t-lg text-[10px] font-bold uppercase tracking-widest">Phần II (Đúng/Sai)</div>
            <div className="col-span-3 py-1 bg-amber-600 text-white rounded-t-lg text-[10px] font-bold uppercase tracking-widest">Phần III (Trả lời ngắn)</div>
          </div>
            ...
            {selectedTopics.map((topic, idx) => (
              <div key={idx} className="grid grid-cols-[2.5fr_repeat(9,1fr)] gap-2 items-center bg-white p-2 rounded-xl border border-gray-200 shadow-sm hover:border-blue-400 transition-all group">
                ...
                <select 
                  ...
                  className="w-full p-2 text-xs border rounded-lg outline-none bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-500 font-medium"
                >
                  <option value="">-- Chọn chuyên đề --</option>
                  {appConfig?.topics?.map((t, i) => (
                    <option key={i} value={t.id}>{t.id} : {t.name}</option>
                  ))}
                </select>

                {/* Ô nhập liệu các phần */}
                {[ 'numMC', 'mcL3', 'mcL4', 'numTF', 'tfL3', 'tfL4', 'numSA', 'saL3', 'saL4' ].map((field) => (
                  <input 
                    ...
                    className={`w-full p-2 border rounded-lg text-center text-xs outline-none focus:ring-2 transition-all ${
                      field.includes('MC') ? 'focus:ring-blue-500' : 
                      field.includes('TF') ? 'focus:ring-emerald-500' : 'focus:ring-amber-500'
                    }`}
                  />
                ))}
              </div>
  ...
  <p className="text-[11px] text-gray-400 italic">
    * Lưu ý: Các dòng không chọn chuyên đề sẽ tự động bị loại bỏ khi lưu.
  </p>
</div>
      </div>

      {/* HÀNG 3: CẤU HÌNH ĐIỂM & NÚT LƯU (FOOTER) */}
      <div className="p-4 bg-gray-50 border-t shadow-[0_-4px_10px_rgba(0,0,0,0.03)]">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div className="flex items-center gap-3 p-3 bg-white rounded-xl border border-blue-100">
            <span className="w-10 h-10 flex items-center justify-center bg-blue-100 text-blue-600 rounded-lg font-bold text-xs">P.I</span>
            <div className="flex-1">
              <label className="block text-[10px] font-bold text-gray-400 uppercase">Điểm mỗi câu</label>
              <input value={maTranForm.scoreMC} onChange={e => setMaTranForm({...maTranForm, scoreMC: e.target.value})} className="w-full font-bold text-blue-700 outline-none" placeholder="0.25"/>
            </div>
          </div>
          <div className="flex items-center gap-3 p-3 bg-white rounded-xl border border-emerald-100">
            <span className="w-10 h-10 flex items-center justify-center bg-emerald-100 text-emerald-600 rounded-lg font-bold text-xs">P.II</span>
            <div className="flex-1">
              <label className="block text-[10px] font-bold text-gray-400 uppercase">Điểm mỗi câu</label>
              <input value={maTranForm.scoreTF} onChange={e => setMaTranForm({...maTranForm, scoreTF: e.target.value})} className="w-full font-bold text-emerald-700 outline-none" placeholder="1.0"/>
            </div>
          </div>
          <div className="flex items-center gap-3 p-3 bg-white rounded-xl border border-amber-100">
            <span className="w-10 h-10 flex items-center justify-center bg-amber-100 text-amber-600 rounded-lg font-bold text-xs">P.III</span>
            <div className="flex-1">
              <label className="block text-[10px] font-bold text-gray-400 uppercase">Điểm mỗi câu</label>
              <input value={maTranForm.scoreSA} onChange={e => setMaTranForm({...maTranForm, scoreSA: e.target.value})} className="w-full font-bold text-amber-700 outline-none" placeholder="0.5"/>
            </div>
          </div>
        </div>

        <div className="flex gap-4">
          <button onClick={() => setIsMatrixOpen(false)} className="flex-1 py-3.5 px-6 border border-gray-300 rounded-xl font-bold text-gray-600 hover:bg-gray-100 active:scale-95 transition-all">Hủy bỏ</button>
          <button 
            ...
            className={`flex-[2] py-3.5 px-6 rounded-xl font-extrabold text-white shadow-lg transition-all flex items-center justify-center gap-3 ${
              loadingMatrix ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-blue-700 to-indigo-800 hover:shadow-blue-500/25 hover:scale-[1.02] active:scale-95'
            }`}
          >
            {loadingMatrix ? (
              <><svg className="animate-spin h-5 w-5 text-white" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> ĐANG LƯU...</>
            ) : (
              <><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" /></svg> XÁC NHẬN LƯU MA TRẬN</>
            )}
          </button>
        </div>
      </div>
