...
//Link Script: https://script.google.com/macros/s/AKfycbwZdb9kvZk6acp9aVkZBvsV-4hXpnQOdHYzOS1jHFmvwPsYcjXz7IDBQ8xuF_PgB3Bkkg/exec
// Lệnh SHIFT + ALT + F để sắp xếp code.gs=============================================================================================
// =====================GV ==========================================================
const SPREADSHEET_ID = "1LlFAI1J0b7YQ84BL674r2kr3wSoW9shgsXSIXVPDypM";
const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
const passReset = "H11111@" // mật khẩu Reset
...
// Khu vực xác minh #01
  const mockEvent = {
    parameter: {
      id: "601.1", // Giả lập ID lớp 6, dạng 01, mức 1
      action: "view"
    }
  };
  doGet(mockEvent);
...
function doGet(e) {
  // Thêm dòng kiểm tra an toàn (Guard Clause)
  if (!e || !e.parameter) {
    Logger.log("Chưa có tham số truyền vào!");
    return ContentService.createTextOutput("Vui lòng truy cập qua URL Web App.");
  }
...
// Khu vực xác minh #05
  ...
  try {
    ...
    const res = (status, message, payload) =>
      ...
      // Edit giáo viên gameshow
      if (action === "updateTeacher") {
  return updateTeacher(e);
}
