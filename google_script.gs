// =====================ADMIN =======================================================
const SPREADSHEET_ID_ADMIN = "1ApgSUHrw_vzlkX8x2GKr3YMgsD9S8fcjl0ITleKZhTk";
...
const passAdmin = "H1111@"
// Lệnh SHIFT + ALT + F để sắp xếp code.gs=============================================================================================
// =====================GV ==========================================================
const SPREADSHEET_ID = "1ApgSUHrw_vzlkX8x2GKr3YMgsD9S8fcjl0ITleKZhTk";
const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
const passReset = "H1111@"
...
function doGet(e) {
  ...
  // load ngân hàng đề
  if (e.parameter.action === "loadQuestions") {    
    var sheetNH = getsheetname(params.mon); 
    ...
    var result = rows.map(function (r) {
      ...
      if (r[5] === "mcq") {
        obj.o = r[8] ? JSON.parse(r[8]) : [];
        obj.a = r[9];
      }

      if (r[5] === "true-false") {
        obj.s = r[8] ? JSON.parse(r[8]) : [];
      }
  ...
  if (action === 'getLG') {
    ...
    var monHienTai = params.mon || "toan";
    var targetSheet = getsheetname(monHienTai);
  ...
  // Lấy môn từ URL, ví dụ: ?action=getAppConfig&mon=toan
  var monTuUrl = e.parameter.mon || "toan"; 
  
  return ContentService.createTextOutput(JSON.stringify({
    status: "success",
    data: getAppConfig(monTuUrl) // Truyền môn vào đây
  })).setMimeType(ContentService.MimeType.JSON);
}

  // THÊM NHÁNH NÀY CHO MA TRẬN
if (action === 'getAppConfigmt') {
  // Lấy môn từ tham số URL (?action=getAppConfigmt&mon=toan)
  var monTuUrl = e.parameter.mon || "toan"; 
  
  return ContentService.createTextOutput(JSON.stringify({
    status: "success",
    data: getAppConfigmt(monTuUrl) // Phải truyền mon vào đây
  })).setMimeType(ContentService.MimeType.JSON);
}
  ...
  if (action == 'deleteQuestionRow') {
    ...
    return ContentService.createTextOutput(JSON.stringify(deleteQuestionRow(rowIdx, mon)))
      .setMimeType(ContentService.MimeType.JSON);
  }
  // -----------------------
  ...
  if (action === 'getQuestionById') {
    ...
    var mon = e.parameter.mon; // Lấy mon từ URL  
    var sheetNH = getsheetname(mon);
    var dataNH = sheetNH.getDataRange().getValues();
    for (var i = 1; i < dataNH.length; i++) {
      if (dataNH[i][3].toString() === id.toString()) {
        return createResponse("success", "OK", {
          idquestion: dataNH[i][3],
          classTag: dataNH[i][4],
          question: dataNH[i][7],
          options: dataNH[i][8],
          answer: dataNH[i][9],
          loigiai: dataNH[i][10],
          datetime: dataNH[i][11]
        });
      }
    }
  ...
  if (action === "getQuestions") {
    ...
    for (var i = 1; i < rows.length; i++) {
      ...
      try {
        ...
        if (!obj.classTag) obj.classTag = rows[i][4];
...
function doPost(e) {
  ...
  if (closeTime && now > closeTime) {
    ...
    // 1. NHÁNH LỜI GIẢI (saveLG) - BẢN KHỚP THEO ID
if (action === 'saveLG') {
  var sheetNH = getsheetname(params.mon);
  var lastRow = sheetNH.getLastRow();
  if (lastRow < 2) return createResponse("error", "Database trống!");

  // Lấy ID ở cột D (cột 4) để tra cứu
  var idValues = sheetNH.getRange(1, 4, lastRow, 1).getValues().flat().map(String);
  var count = 0;
  ...
  data.forEach(function (item) {
    ...
    if (rowIndex !== -1) {
      ...
      sheetNH.getRange(rowNum, 11).setValue(contentLG);
      count++;
    }
  });
  ...
  return createResponse("success", "Đã cập nhật " + count + " lời giải qua Web thành công!");
    ...
    if (action === 'saveQuestions') {
      ...
      var now = new Date();
      var mon = e.parameter.mon; 
      var sheetNH = getsheetname(mon);
      if (!sheetNH) return createResponse("error", "Không tìm thấy sheet môn: " + params.mon);
      var dataD = sheetNH.getRange("D:D").getValues();
      var lastRowD = 0;
      for (var i = dataD.length - 1; i >= 0; i--) {
      if (dataD[i][0] !== "" && dataD[i][0] !== null) {
      lastRowD = i + 1;
      break;
      }
    }
    ...
    // Cột 4 chính là cột D
    sheetNH.getRange(startRow, 4, rows.length, rows[0].length)
      .setValues(rows);
    ...
    sheetNH.getRange(startRow, 8, rows.length, 4).setWrap(true);
        }
      var lastRow = sheetNH.getLastRow();
      // Tự chỉnh chiều cao từ dòng 2 trở xuống
      if (lastRow > 1) {
...
// Hàm lấy cấu hình chuyên
function getAppConfig(mon) {  
  // Bây giờ ta dùng biến 'mon' vừa nhận được thay vì 'params.mon'
  var sheetCD = getsheetname(mon);
...
function findDuplicateQuestions() {  
  var sheetNH = getsheetname(params.mon);
  const data = sheetNH.getDataRange().getValues();
...
function deleteQuestionRow(rowIdx, mon) {
  try {
    var sheetNH = getsheetname(mon);
    if (!sheetNH) return { status: "error", message: "Không tìm thấy sheet" };
    sheetNH.deleteRow(parseInt(rowIdx));
...
function updateQuestion(payload) {
  try {
    ...
    const mon = payload.mon;
    var sheetNH = getsheetname(mon);
    const fullData = sheetNH.getDataRange().getValues();
    ...
    for (var i = 1; i < fullData.length; i++) {
      ...
      if (fullData[i][0].toString() === data.id.toString()) {
        ...
        Object.keys(data).forEach(key => {
          ...
          if (colIdx !== -1) {
            sheetNH.getRange(rowNum, colIdx + 1).setValue(data[key]);
          }
        });
...
function getDataByMon(mon) { 
  
  // 1. Chuyển mon về chữ thường để khớp với tên sheet (ví dụ: NHtoan)
  const cleanMon = String(mon || "").toLowerCase().trim();
  const sheetName = "NH" + cleanMon; 
  
  const sheet = ssAdmin.getSheetByName(sheetName);
  
  if (!sheet) {
    return { 
      status: "error", 
      message: "Không tìm thấy sheet ngân hàng: " + sheetName 
    };
  }
  ...
  if (fullData.length <= 1) {
    return { status: "success", data: [], message: "Sheet rỗng" };
  }

  // 3. Loại bỏ dòng tiêu đề (row 0) và trả về dữ liệu thực tế
  const dataWithoutHeader = fullData.slice(1);
  
  return { 
    status: "success", 
    data: dataWithoutHeader,
    count: dataWithoutHeader.length 
  };
}
// ==== Hàm lấy tên sheet
function getsheetname(mon) {
  const cleanMon = String(mon || "toan").toLowerCase().trim();
  const sheetName = "NH" + cleanMon;
  return ssAdmin.getSheetByName(sheetName);
}
